...
Gitpull() {
  git clone https://github.com/VCStardust/Openaibot.git || (
    echox yellow "Git failed,try pull from mirror"
    git clone https://gitclone.com/github.com/VCStardust/Openaibot.git
  )
}
...
run() {
  ...
  #    now=$version
  #    (curl -s https://raw.fastgit.org/VCStardust/Openaibot/main/project.info) && declare<(curl -s https://raw.fastgit.org/VCStardust/Openaibot/main/project.info)
  #    new=$version
  #    if [[ $new ]]; then
  #       if [[ $((new)) -gt $((now)) ]]; then
